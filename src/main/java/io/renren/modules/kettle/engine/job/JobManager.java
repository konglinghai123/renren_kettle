...
package io.renren.modules.kettle.engine.job;
...
import io.renren.modules.kettle.engine.util.Underline2Camel;
...
import my.study.constants.UtilConst;
import my.study.kettle.common.KuConst;
import my.study.utils.DateUtil;
...
public class JobManager extends AbsJob {
    ...
    */
    ...
    private static KettleLogListener logListener = null;
    ...
    */
    ...
    protected void process() throws Exception {
        ...
        for(JSONObject jsonjob:restartList){
            startJob(Underline2Camel.Json2Object(jsonjob,VJobEntity.class));
            //已重启次数+1
    ...
    */
    public static void init(String view, String projectCode,KettleLogListener logListener){
        setJobViewName(view);
        ...
        for(JSONObject job:list){
            ...
            try {
                status = startJob(Underline2Camel.Json2Object(job,VJobEntity.class));
            } catch (Exception e) {
    ...
    * 启动作业 <br/>
    * @author jingma
    * @param VJobEntity 作业对象
    * @return
    * @throws Exception
    */
    public static String startJob(VJobEntity VJobEntity) throws Exception {

        Date start = new Date();
        JobMeta jm = KettleUtils.loadJob(VJobEntity.getIdJob());
        log.info("加载作业总耗时："+(new Date().getTime()-start.getTime())+","+VJobEntity.toString());
        Map<String, JSONObject> paramMap = kettledb.findMap("ocode","select * from job_params jp where jp.id_job=?", VJobEntity.getIdJob());
        ...
        jsonjobMap.put(job, JSON.parseObject(JSON.toJSONString(VJobEntity)));
        return startJob(job);
    }
    ...
    */
    public static int writeJobLog(Job job) {
        ...
        //如果不写日志到文件
        logListener.handleMsg(job,joblogStr);
        if(isWriteLogFile()){
    ...
    */
    ...
    public static void setLogListener(KettleLogListener logListener){
        if(logListener !=null){
            JobManager.logListener = logListener;
        }
    }
    /**
     * @return projectCode 
